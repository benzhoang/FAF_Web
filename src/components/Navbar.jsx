...
══════════════════════════════════════════════════════ */
const MessagesDropdown = ({ onClose, navigate }) => {
    ...
    return (
        <div className="absolute right-0 top-full mt-2 w-[360px] bg-[#090e17]/95 backdrop-blur-xl rounded-xl shadow-[0_0_30px_rgba(6,182,212,0.15)] border border-cyan-500/20 z-50 overflow-hidden animate-[dropIn_.18s_ease-out]"
            ...
            <div className="px-5 pt-5 pb-2">
                <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-3">
                    <h2 className="text-[14px] font-black text-cyan-500 uppercase font-mono tracking-widest text-shadow-glow-cyan flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                        SYS.COMMS
                    </h2>
                    <button
                        onClick={() => { navigate('/messages'); onClose(); }}
                        className="w-8 h-8 rounded shrink-0 bg-[#02040a] border border-cyan-500/30 hover:border-cyan-400 hover:bg-cyan-900/30 flex items-center justify-center transition-colors text-cyan-400 group"
                        title="OPEN_TERMINAL"
                    >
                ...
                <div className="relative mb-3 group">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2 bg-[#02040a] border border-slate-700 focus:border-cyan-500/50 rounded text-xs font-mono text-slate-300 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 placeholder-slate-600 uppercase tracking-wide transition-all"
                    />
                </div>
                        ...
                        {filtered.map(conv => {
                            ...
                            return (
                                <button
                                    key={conv.id}
                                    onClick={() => { navigate('/messages'); onClose(); }}
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 hover:bg-cyan-900/10 transition-colors rounded border-l-2 mb-1 group text-left ${
                                        isUnread ? 'border-cyan-400 bg-cyan-950/20' : 'border-transparent hover:border-slate-700'
                                    }`}
                                >
                                    {/* Avatar */}
                                    <div className="relative flex-shrink-0">
                                        <div className={`w-10 h-10 rounded border flex items-center justify-center text-white font-black text-xs font-mono overflow-hidden ${
                                            isUnread ? 'bg-cyan-950 border-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.3)]' : 'bg-[#02040a] border-slate-700'
                                        }`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between gap-1">
                                            <p className={`text-xs truncate font-mono uppercase tracking-wide ${isUnread ? 'font-black text-cyan-300' : 'font-bold text-slate-300'}`}>
                                                {name}
                                            </p>
                                        ...
                                        <div className="flex items-center justify-between gap-1 mt-1">
                                            <p className={`text-[10px] font-mono truncate ${isUnread ? 'text-cyan-100' : 'text-slate-500'}`}>
                                                {conv.last_message ? `> ${conv.last_message}` : '> AWAITING_INPUT'}
                                            </p>
                                            {isUnread && (
                                                <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 flex-shrink-0 shadow-[0_0_8px_rgba(34,211,238,1)] animate-pulse" />
                                            )}
                                        </div>
                                    </div>
                                </button>
            ...
            <div className="border-t border-slate-800 bg-[#02040a]/50 p-2">
                <button
                    onClick={() => { navigate('/messages'); onClose(); }}
                    className="w-full py-2.5 text-center text-[10px] font-black font-mono tracking-widest uppercase text-cyan-500 hover:text-cyan-300 hover:bg-cyan-950/30 rounded transition-colors"
                >
...
══════════════════════════════════════════════════════ */
const NotificationsDropdown = ({ onClose, navigate, onMarkAllRead }) => {
    ...
    const NotifItem = ({ n }) => (
        <button
            ...
            className={`w-full flex items-start gap-3 px-3 py-2.5 hover:bg-indigo-900/10 transition-colors rounded border-l-2 mb-1 group text-left ${
                !n.is_read ? 'border-indigo-400 bg-indigo-950/20' : 'border-transparent hover:border-slate-700'
            }`}
        >
            {/* Icon / Avatar */}
            <div className="relative flex-shrink-0 mt-0.5">
                <div className={`w-8 h-8 rounded border flex items-center justify-center text-sm overflow-hidden ${
                    !n.is_read ? 'bg-indigo-950 border-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.3)] text-indigo-300' : 'bg-[#02040a] border-slate-700 text-slate-500'
                }`}>
                    {n.icon || '(!)'}
                </div>
            </div>
            ...
            {/* Text */}
            <div className="flex-1 min-w-0">
                <p className={`text-[11px] font-mono leading-tight uppercase tracking-wide ${!n.is_read ? 'text-indigo-100 font-bold' : 'text-slate-400'}`}>
                    {n.message || n.title || 'SYS.ALERT_RECEIVED'}
                </p>
                <p className={`text-[9px] mt-1 font-mono tracking-widest ${!n.is_read ? 'text-indigo-400 font-bold' : 'text-slate-600'}`}>
    ...
    return (
        <div className="absolute right-0 top-full mt-2 w-[380px] bg-[#090e17]/95 backdrop-blur-xl rounded-xl shadow-[0_0_30px_rgba(99,102,241,0.15)] border border-indigo-500/20 z-50 overflow-hidden animate-[dropIn_.18s_ease-out]"
             style={{ transformOrigin: 'top right' }}>
            ...
            <div className="px-5 pt-5 pb-2">
                <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-3">
                    <h2 className="text-[14px] font-black text-indigo-500 uppercase font-mono tracking-widest flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                        SYS.LOGS
                    </h2>
                    <button
                        onClick={handleMarkAll}
                        className="text-[9px] font-black font-mono tracking-widest uppercase text-slate-500 hover:text-indigo-400 transition-colors"
                    >
                        ACKNOWLEDGE_ALL
                    </button>
                </div>
            ...
            <div className="max-h-[360px] overflow-y-auto custom-scrollbar px-2 py-1">
                {loading ? (
                    ...
                    <div className="py-10 text-center flex flex-col items-center">
                        <svg className="w-8 h-8 text-slate-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
            ...
            <div className="border-t border-slate-800 bg-[#02040a]/50 p-2">
                <button
                    ...
                    className="w-full py-2.5 text-center text-[10px] font-black font-mono tracking-widest uppercase text-indigo-500 hover:text-indigo-300 hover:bg-indigo-950/30 rounded transition-colors"
                >
                    ACCESS_FULL_ARCHIVE
                </button>
            </div>
...
const Navbar = () => {
    ...
    return (
        <nav className="sticky top-0 z-50 w-full border-b" style={{ background: 'rgba(2,6,23,0.95)', borderColor: 'rgba(6,182,212,0.15)', backdropFilter: 'blur(12px)' }} ref={panelRef}>
            {/* Top scanning line for extra cyberpunk feel */}
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-cyan-500/0 via-cyan-400/50 to-cyan-500/0 opacity-50" />
            ...
            <div className="max-w-[1300px] mx-auto px-4 h-[56px] flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                    <NavLink to="/" aria-label="Home" className="relative flex items-center group">
                        <div className="absolute inset-0 rounded bg-cyan-500/0 blur group-hover:bg-cyan-500/20 transition-all duration-300" />
                        <img
                            src={FAFLogo}
                            alt="FAF"
                        ...
                        />
                        <span className="ml-2 font-black font-mono text-[14px] tracking-widest text-white uppercase group-hover:text-cyan-400 transition-colors">FAF<span className="text-cyan-500">_</span>NET</span>
                    </NavLink>


                    <div className="hidden lg:block relative group">
                        <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
                            <svg className="h-3.5 w-3.5 text-slate-600 group-focus-within:text-cyan-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                            </svg>
                        </div>
                        <input
                            className="w-[240px] pl-9 pr-3 py-1.5 rounded bg-[#02040a] border border-slate-800 text-[11px] placeholder-slate-600 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 transition-all font-mono text-slate-300 uppercase tracking-widest"
                ...
                {user && (
                    <div className="flex items-center h-full flex-1 justify-center gap-1 sm:gap-2">
                        {[
                            ...
                            const active = exact ? loc === to : loc.startsWith(to) && to !== '/';
                            return (
                                <NavLink
                                    key={to}
                                    to={to}
                    ...
                    {user ? (
                        <>
                            ...
                            <div className="relative">
                                <IconBtn id="messages" badge={unreadCount} title="SYS.COMMS" color="cyan">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                                    </svg>
                                </IconBtn>
                                {openPanel === 'messages' && (
                                    <MessagesDropdown onClose={close} navigate={navigate} />
                                )}
                            </div>

                            {/* Notifications */}
                            <div className="relative">
                                <IconBtn id="notifications" badge={unreadNotifications} title="SYS.LOGS" color="indigo">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className={`w-9 h-9 rounded bg-[#02040a] border flex items-center justify-center text-cyan-400 font-black font-mono text-xs transition-all overflow-hidden ${
                                        openPanel === 'profile' ? 'border-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.3)]' : 'border-slate-700 hover:border-cyan-500/50'
                                    }`}
                                >
                                    {user?.avatar_url
                                        ? <img src={user.avatar_url} alt="av" className="w-full h-full object-cover mix-blend-luminosity hover:mix-blend-normal transition-all" />
                                        : getInitials(user)}
                                </button>

                                {openPanel === 'profile' && (
                                    <div className="absolute right-0 top-full mt-2 w-[240px] rounded bg-[#090e17]/95 backdrop-blur-xl shadow-[0_0_30px_rgba(6,182,212,0.1)] z-50 overflow-hidden animate-[dropIn_.18s_ease-out] border border-cyan-500/30">
                                        {/* Top neon line */}
                                        ...
                                        <div className="px-4 pt-5 pb-3 border-b border-slate-800/80 bg-slate-900/30">
                                            <div className="flex flex-col mb-3">
                                                <p className="font-black text-white text-sm uppercase tracking-widest font-mono text-shadow-glow-cyan leading-tight">{user?.full_name || 'UNKNOWN_NODE'}</p>
                                                <p className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5 mt-1">
                                                    <span className={`w-1.5 h-1.5 rounded-full ${user?.role === 'employer' ? 'bg-purple-500 shadow-[0_0_5px_rgba(168,85,247,0.8)]' : 'bg-cyan-500 shadow-[0_0_5px_rgba(34,211,238,0.8)]'}`}></span>
                                                    {user?.role === 'employer' ? 'CLIENT_OP' : 'WORKER_OP'}
                                                </p>
                                            </div>
                                            <button
                                                onClick={() => { close(); navigate(user?.role === 'employer' ? '/task-owner/profiles' : '/dashboard'); }}
                                                className="w-full py-1.5 rounded font-black text-[9px] tracking-widest uppercase font-mono transition-all bg-[#02040a] hover:bg-cyan-950 text-cyan-500 hover:text-cyan-300 border border-slate-700 hover:border-cyan-500/50"
                                            >
                                                VIEW_DETAILS
                                            </button>
                                        </div>
                                        ...
                                        <div className="py-1">
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    className="w-full flex items-center gap-2.5 px-4 py-2 text-[10px] font-black font-mono tracking-widest uppercase text-slate-400 hover:text-cyan-400 hover:bg-cyan-950/20 transition-colors border-l-2 border-transparent hover:border-cyan-400"
                                                >
